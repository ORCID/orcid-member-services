...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
    ...
    ResponseEntity<Problem> handleMemberNotFoundException(MemberNotFoundException ex, NativeWebRequest request) {
        Problem problem = Problem.builder().withType(ErrorConstants.CONSTRAINT_VIOLATION_TYPE).withTitle("Member not found").withStatus(Status.BAD_REQUEST)
                .with(MESSAGE_KEY, ErrorConstants.ERR_MEMBER_NOT_FOUND).with(PARAMS, ex.getParamMap().get("params")).build();
        return create(ex, problem, request);
    }
