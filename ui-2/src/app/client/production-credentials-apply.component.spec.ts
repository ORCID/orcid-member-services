...
describe('ProductionCredentialsApplyComponent', () => {
  ...
  beforeEach(() => {
    ...
    const apiCredentialsServiceSpy = jasmine.createSpyObj('ApiCredentialsService', [
      'submitProductionCredentialsApplication',
    ])
    apiCredentialsServiceSpy.submitProductionCredentialsApplication.and.returnValue(of(void 0))
  ...
  it('should submit the mapped form data, then broadcast a toast and navigate to the list', () => {
    ...
    expect(apiCredentialsService.submitProductionCredentialsApplication).toHaveBeenCalledWith({
      systemIntegrationType: 'IN_HOUSE',
      authenticateIdsAnswer: 'YES',
      integrationDisplayName: 'My App',
      integrationHomepageUrl: 'https://example.org',
      integrationDescription: 'A test integration',
      redirectUris: ['https://example.org/callback'],
    })
  ...
  it('should leave the user on the form and show an error when submission fails', () => {
    apiCredentialsService.submitProductionCredentialsApplication.and.returnValue(
      throwError(() => new Error('The service is unavailable'))
    )
    component.applyForm.setValue({
      integrationType: 'VENDOR',
      authenticateOrcidIds: 'NO',
      displayName: 'My App',
      homepageUrl: 'https://example.org',
      description: 'A test integration',
      redirectUris: null,
