...
Cypress.Commands.add('processPasswordForm', newPasswordFieldId => {
  ...
  cy.get(newPasswordFieldId)
    .clear()
    .type(credentials.shortPassword);
...
Cypress.Commands.add('addMember', (salesforceId, assertionServiceEnabled, isConsortiumLead, code, title) => {
  cy.log('addMember');
  cy.getCookie('XSRF-TOKEN').then(csrfCookie => {
    cy.request({
      method: 'POST',
...
Cypress.Commands.add('updateMember', (salesforceId, assertionServiceEnabled, isConsortiumLead, code, title) => {
  cy.log('updateMember');
  cy.getCookie('XSRF-TOKEN').then(csrfCookie => {
    cy.request({
      method: 'PUT',
...
Cypress.Commands.add('validateMember', (salesforceId, assertionServiceEnabled, isConsortiumLead, code, title) => {
