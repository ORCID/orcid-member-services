...
Cypress.Commands.add('signin', (email, password) => {
  cy.visit('./');
  cy.get('#username').clear().type(email).get('#password').type(password).get('button').filter('[type="submit"]').click();
...
Cypress.Commands.add('processPasswordForm', newPasswordFieldId => {
  ...
  cy.get('small').filter('[jhitranslate="global.messages.validate.newpassword.minlength.string"]').should('exist');
  cy.get('small').filter('[jhitranslate="global.messages.validate.confirmpassword.minlength.string"]').should('exist');
  // fix password
  cy.get(newPasswordFieldId).clear().type(credentials.password);
  // enter invalid confirmation password
  cy.get('#confirmPassword').clear().type(credentials.wrongConfirmationPasssword);
  // make sure you can't activate account
  cy.get('button').filter('[type="submit"]').click();
  // check for confirmation error message
  cy.get('div').filter('[jhitranslate="global.messages.error.dontmatch.string"]').should('exist');
...
Cypress.Commands.add('fetchLinkAndGrantPermission', email => {
  ...
  cy.get('tbody')
    ...
    .last()
    .within(() => {
      cy.get('a').filter('[jhitranslate="gatewayApp.assertionServiceAssertion.details.string"]').click();
    });
  cy.get('.jh-entity-details').within(() =>
    cy.get('button').filter('[jhitranslate="gatewayApp.assertionServiceAssertion.copyClipboard.string"]').click()
  );
  ...
  // *ADD ID
  cy.get('.mt-5').within(() => {
    cy.get('h2').filter('[jhitranslate="landingPage.success.thanks.string"]').should('exist');
  });
});
