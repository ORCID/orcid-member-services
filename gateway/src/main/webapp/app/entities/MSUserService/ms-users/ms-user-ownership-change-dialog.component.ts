...
@Component({
  selector: 'jhi-ms-user-ownership-change-dialog',
  templateUrl: './ms-user-ownership-change-dialog.component.html'
})
export class MSUserOwnershipChangeDialogComponent {
  msUser: IMSUser;
  ...
  confirmOwnershipChange(id: string) {
...
@Component({
  selector: 'jhi-ms-user-ownership-change-popup',
  template: ''
})
export class MSUserDeletePopupComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.activatedRoute.data.subscribe(({ msUser }) => {
      setTimeout(() => {
        this.ngbModalRef = this.modalService.open(MSUserOwnershipChangeDialogComponent as Component, { size: 'lg', backdrop: 'static' });
