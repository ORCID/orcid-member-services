...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
    ...
    public ResponseEntity<Problem> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, @Nonnull NativeWebRequest request) {
        ...
        List<FieldErrorVM> fieldErrors = result.getFieldErrors().stream()
                .map(f -> new FieldErrorVM(f.getObjectName(), f.getField(), f.getCode() != null ? f.getCode().toLowerCase() : f.getCode())).collect(Collectors.toList());
    ...
    ResponseEntity<Problem> handleEmailAlreadyUsedException(EmailAlreadyUsedException ex, NativeWebRequest request) {
        Problem problem = Problem.builder().withType(ErrorConstants.CONSTRAINT_VIOLATION_TYPE).withTitle("Assertion email already used").withStatus(Status.BAD_REQUEST)
                .with(MESSAGE_KEY, ErrorConstants.ERR_EMAIL_AFFILIATION_USED).with(PARAMS, ex.getParamMap().get("params")).build();
        return create(ex, problem, request);
    }
}
