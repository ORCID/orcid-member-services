...
public class AssertionsForEditCsvWriter extends CsvWriter {
    ...
    private CSVPrinter getCSVPrinterWithHeaders(StringBuffer buffer) throws IOException {
        return new CSVPrinter(buffer,
                CSVFormat.DEFAULT.withHeader("email", "affiliation-section", "department-name", "role-title", "start-date", "end-date", "org-name", "org-country",
                        "org-city", "org-region", "disambiguation-source", "disambiguated-organization-identifier", "external-id", "external-id-type", "external-id-url",
                        "url", "id"));
    }
